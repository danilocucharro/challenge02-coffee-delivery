...
export function Checkout() {
  ...
  const handleSubmitOrder = () => {
    if(address.numero === 0 || address.numero === undefined){
      toast.info("Antes de prosseguir, confirme o seu endereço!", {
        duration: 1500
      })
    }
  ...
  return(
    <div className="flex flex-col items-center bg-background h-lvh">
      ...
      <section className="flex w-full max-w-[1420px] px-40 justify-between mt-28 gap-8 max-mobile:px-5 max-mobile:flex-col">
        ...
        <div className="mt-8 max-mobile:w-full">
          <h2 className="text-lg font-baloo2 font-bold mb-3 max-mobile:text-2xl">Cafés selecionados</h2>
          <div className="w-[448px] max-mobile:w-full p-10 bg-base-card rounded-tl-md gap-6 rounded-tr-[44px] rounded-br-md rounded-bl-[44px]">
            {cartItems.length === 0 ? (
                ...
                {cartItems.map((item) => (
                  <div key={item.coffeeName}>
                    <CartCoffeeCard
                      ...
                      coffeeAmount={item.coffeeAmount}
                    />
